...
# * If an elf is responsible for more than one house, he does not deliver and, instead, delegates
#   two elves (one for half of the houses, another for the other half). Delegation keeps happening
...
# When dealing with recursive functions, keep in mind that each recursive call has its own 
# execution context, so to maintain state during recursion you have to either:

# 1. Thread the state through each recursive call so that the current state is part of the current
# call's execution context (i.e. pass the updated current state to each recursive call as
# arguments)
...
#    needed to understand each piece, the better. Since global variables can be modified or read in
#    a wide variety of places, the sloppy use of can destroy locality. Nevertheless, there are
#    times when they are just what is needed." — John V. Guttag (Introduction to Computation and
...
# Assume that you have only an empty list at your disposal, and the only operation you can perform
# on it is this:
...
# The recursive function's structure can often be modeled after the definition of the recursive
# data structure it takes as an input. For example, calculating the sum of all the elements of a
# list recursively:
